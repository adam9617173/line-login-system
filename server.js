...
function getUsers() {
  if (!fs.existsSync(DB_FILE)) {
    console.log('[getUsers] File not exists, returning empty');
    return [];
  }
  const data = JSON.parse(fs.readFileSync(DB_FILE, 'utf8'));
  console.log('[getUsers] Returning ' + data.length + ' users');
  return data;
}
...
function getCards() {
  if (!fs.existsSync(CARDS_FILE)) {
    console.log('[getCards] File not exists, returning empty');
    return [];
  }
  const data = JSON.parse(fs.readFileSync(CARDS_FILE, 'utf8'));
  console.log('[getCards] Returning ' + data.length + ' cards');
  return data;
}

function saveCards(cards) {
  fs.writeFileSync(CARDS_FILE, JSON.stringify(cards, null, 2));
  console.log('[saveCards] Saved ' + cards.length + ' cards');
}
...
app.get('/callback', async (req, res) => {
  console.log('[GET /callback] Line callback, code:', req.query.code ? 'yes' : 'no');
  ...
  try {
    ...
    console.log('[callback] User: ' + displayName + ', ID: ' + userId);
...
app.post('/api/users/toggle', (req, res) => {
  console.log('[POST /api/users/toggle]', req.body);
...
app.get('/api/cards', (req, res) => {
  console.log('[GET /api/cards] Returning cards');
  res.json(getCards());
});

app.post('/api/cards', (req, res) => {
  console.log('[POST /api/cards]', req.body);
...
app.post('/api/cards/delete', (req, res) => {
  console.log('[POST /api/cards/delete]', req.body);
  const { lineId } = req.body;
