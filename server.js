...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    if (userCard) {
      res.send('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>我的名片</title><style>body { font-family: -apple-system, sans-serif; background: linear-gradient(135deg, #667eea, #764ba2); min-height: 100vh; display: flex; justify-content: center; align-items: center; }.card { background: white; border-radius: 20px; padding: 2rem; width: 320px; text-align: center; box-shadow: 0 10px 40px rgba(0,0,0,0.3); }.avatar { width: 100px; height: 100px; border-radius: 50%; margin-bottom: 1rem; }.name { font-size: 1.5rem; font-weight: bold; color: #1a1a2e; margin-bottom: 0.5rem; }.title { color: #667eea; margin-bottom: 1rem; }.info { text-align: left; margin: 1rem 0; }.info p { margin: 0.5rem 0; color: #666; }.info span { color: #333; font-weight: 500; }.qr { margin-top: 1rem; padding-top: 1rem; border-top: 1px solid #eee; }</style></head><body><div class="card"><img src="' + pictureUrl + '" class="avatar"><div class="name">' + displayName + '</div><div class="title">' + (userCard.title || '會員') + '</div><div class="info"><p><span>公司：</span>' + (userCard.company || '-') + '</p><p><span>電話：</span>' + (userCard.phone || '-') + '</p><p><span>Email：</span>' + (userCard.email || '-') + '</p></div><div class="qr"><img src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=https://line.me/ti/p/~' + (userCard.lineId || userId) + '" alt="LINE QR"></div></div></body></html>');
    } else {
      res.send('<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>建立 LINE 名片</title><style>body { font-family: -apple-system, sans-serif; background: linear-gradient(135deg, #667eea, #764ba2); min-height: 100vh; display: flex; justify-content: center; align-items: center; }.card { background: white; border-radius: 20px; padding: 2rem; width: 340px; }h1 { text-align: center; color: #1a1a2e; margin-bottom: 1.5rem; }input, textarea { width: 100%; padding: 0.8rem; margin-bottom: 1rem; border: 1px solid #ddd; border-radius: 8px; font-size: 1rem; }button { width: 100%; padding: 1rem; background: #06C755; color: white; border: none; border-radius: 8px; font-size: 1.1rem; cursor: pointer; }button:hover { background: #05b54c; }</style></head><body><div class="card"><h1>📇 建立 LINE 名片</h1><form id="cardForm"><input type="text" id="title" placeholder="職稱（例：工程師）" required><input type="text" id="company" placeholder="公司名稱"><input type="tel" id="phone" placeholder="電話"><input type="email" id="email" placeholder="Email"><button type="submit">建立名片</button></form></div><script>document.getElementById("cardForm").onsubmit=async function(e){e.preventDefault();var data={lineId:"' + userId + '",title:document.getElementById("title").value,company:document.getElementById("company").value,phone:document.getElementById("phone").value,email:document.getElementById("email").value};await fetch("/api/cards",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(data)});window.location.reload();};</script></body></html>');
    }
...
// API: 切換用戶權限
app.post('/api/users/toggle', requireAdmin, (req, res) => {
...
// API: 刪除名片
app.post('/api/cards/delete', requireAdmin, (req, res) => {
  const { lineId } = req.body;
