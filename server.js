...
app.get('/' + ADMIN_PATH, (req, res) => {
  res.send('<!DOCTYPE html><html lang="zh-TW"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>用戶管理 - 浩茂AI</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:-apple-system,BlinkMacSystemFont,sans-serif;background:#f5f5f5;}.header{background:linear-gradient(135deg,#1a1a2e,#302b63);color:white;padding:1rem 2rem;}.tabs{display:flex;gap:1rem;padding:1rem 2rem;background:white;border-bottom:1px solid #eee;}.tab{padding:0.5rem 1rem;border-radius:6px;cursor:pointer;color:#666;}.tab.active{background:#667eea;color:white;}.container{max-width:1200px;margin:2rem auto;padding:0 1rem;}.card{background:white;border-radius:12px;padding:1.5rem;margin-bottom:1rem;box-shadow:0 2px 8px rgba(0,0,0,0.1);}table{width:100%;border-collapse:collapse;}th,td{padding:1rem;text-align:left;border-bottom:1px solid #eee;}th{background:#f8f9fa;font-weight:600;}.status-allowed{color:#06C755;font-weight:bold;}.status-pending{color:#ffa502;font-weight:bold;}img{width:40px;height:40px;border-radius:50%;}.empty{text-align:center;padding:2rem;color:#999;}.hidden{display:none;}</style></head><body><div class="header"><h1>👾 用戶管理</h1></div><div class="tabs"><div class="tab active" onclick="switchTab(\'users\')">用戶列表</div><div class="tab" onclick="switchTab(\'cards\')">名片管理</div></div><div class="container"><div id="usersPanel"><div class="card"><table><thead><tr><th>頭像</th><th>名稱</th><th>LINE ID</th><th>狀態</th><th>註冊時間</th></tr></thead><tbody id="userList"><tr><td colspan="5" class="empty">載入中...</td></tr></tbody></table></div></div><div id="cardsPanel" class="hidden"><div class="card"><table><thead><tr><th>頭像</th><th>名稱</th><th>職稱</th><th>公司</th></tr></thead><tbody id="cardList"><tr><td colspan="4" class="empty">載入中...</td></tr></tbody></table></div></div></div><script>function switchTab(tab){document.querySelectorAll(".tab").forEach(function(t){t.classList.remove("active");});document.querySelector(".tab:nth-child("+(tab==="users"?1:2)+")").classList.add("active");document.getElementById("usersPanel").classList.toggle("hidden",tab!=="users");document.getElementById("cardsPanel").classList.toggle("hidden",tab!=="cards");if(tab==="users"){loadUsers();}else{loadCards();}}function h(t){return String(t||"").replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'"'"'/g,"&#039;");}async function loadUsers(){try{var res=await fetch("/api/users");var users=await res.json();var tbody=document.getElementById("userList");if(users.length===0){tbody.innerHTML="<tr><td colspan=5 class=empty>尚有用戶</td></tr>";return;}tbody.innerHTML=users.map(function(u){var avatar=u.pictureUrl?"<img src=\\""+h(u.pictureUrl)+"\\">":"👤";var status=u.allowed?"<span class=status-allowed>✅ 已開通</span>":"<span class=status-pending>⏳ 審核中</span>";return"<tr><td>"+avatar+"</td><td>"+h(u.displayName)+"</td><td><code>"+h(u.lineId)+"</code></td><td>"+status+"</td><td>"+new Date(u.createdAt).toLocaleString()+"</td></tr>";}).join("");}catch(e){document.getElementById("userList").innerHTML="<tr><td colspan=5 class=empty>載入失敗</td></tr>";}}async function loadCards(){try{var res=await fetch("/api/cards");var cards=await res.json();var res2=await fetch("/api/users");var users=await res2.json();var tbody=document.getElementById("cardList");if(cards.length===0){tbody.innerHTML="<tr><td colspan=4 class=empty>尚有名片</td></tr>";return;}tbody.innerHTML=cards.map(function(c){var user=users.find(function(u){return u.lineId===c.lineId;});var avatar=user&&user.pictureUrl?"<img src=\\""+h(user.pictureUrl)+"\\">":"👤";return"<tr><td>"+avatar+"</td><td>"+(user?h(user.displayName):"Unknown")+"</td><td>"+h(c.title||"-")+"</td><td>"+h(c.company||"-")+"</td></tr>";}).join("");}catch(e){document.getElementById("cardList").innerHTML="<tr><td colspan=4 class=empty>載入失敗</td></tr>";}}loadUsers();</script></body></html>');
});
